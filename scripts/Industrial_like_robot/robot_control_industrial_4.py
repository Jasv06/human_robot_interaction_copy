...
def main():
	...
	while not rospy.is_shutdown():
		...
		if number_of_hands == 1 and hand_status < 0.8 and hand_life >= 0.5  and emergencia == 1: #and palm_pointing > 0:
			...
			if x_rate <= 3 and x_rate >= -3 and y_rate <= 3 and y_rate >= -3 and z_rate <= 3 and z_rate >= -3 and hand_life > 3 and x_robot_control <= (x_robot_control_old + 0.01) and x_robot_control >= (x_robot_control_old - 0.01) and y_robot_control <= (y_robot_control_old + 0.01) and y_robot_control >= (y_robot_control_old - 0.01)  and z_robot_control <= (z_robot_control_old + 0.01) and z_robot_control >= (z_robot_control_old - 0.01):
